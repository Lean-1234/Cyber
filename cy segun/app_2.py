...
@app.route('/api/personajes/eliminar', methods=['POST'])
def eliminar_personaje():
    try:
        ...
        if not personaje_id or not usuario_id:
            return jsonify({"status": "error", "message": "Faltan datos obligatorios."}), 400

        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM personajes WHERE id = %s AND usuario_id = %s", (personaje_id, usuario_id))
        conn.commit()
        cursor.close()
        conn.close()

        return jsonify({"status": "success", "message": "Personaje eliminado correctamente."}), 200
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
